...
$(window).on('load', function(){
	...
	function textAnimation() {
		var waypoints = $('.animation-item').waypoint({
			handler: function() {
				TweenMax.to($(this.element), 0.9 , {ease: Power4.easeOut, x: '0%'});
			},
			offset: '90%'
		})
	}
	...
	setTimeout(function() {
		...
		if ($('.js-scroll-img').length) {
			revealerAnimationInit(400);
		}
...
$(document).ready(function() {
	...
	$(".js-slider-animation-wrap .js-next-slide").on('click' , function(){
		if (!sliderInAction) {
			sliderInAction = true;
			...
			// hide
			$(thisItem).closest('.slider-animation-wrap').find('.js-movement-slider-item').each(function(index){
				if (index%2 === 0) {
					TweenMax.to($(this), normalSpeed, {ease: Power4.easeOut, x: '-100%'});
				} else {
					TweenMax.to($(this), delaySpeed, {ease: Power3.easeOut, x: '-100%', delay: '0.1'});
				}
			});
			...
			setTimeout(function(){
				$(thisItem).closest('.slider-animation-wrap').find('.js-movement-slider-item').each(function(index){
					if (index%2 === 0) {
						TweenMax.to($(this), normalSpeed, {ease: Power4.easeOut, x: '0%'});
					} else {
						TweenMax.to($(this), delaySpeed, {ease: Power3.easeOut, x: '0%', delay: '0.1'});
					}
				});
			}, slideShow);
		}
	...
	$(".slider-animation-wrap .js-prev-slide").on('click' , function(){
		if (!sliderInAction) {
			sliderInAction = true;
			...
			$(thisItem).closest('.slider-animation-wrap').find('.js-movement-slider-item').each(function(index){
				if (index%2 === 0) {
					TweenLite.to($(this), normalSpeed, {ease: Power4.easeOut, x: '-100%'});
				} else {
					TweenLite.to($(this), delaySpeed, {ease: Power3.easeOut, x: '-100%', delay: '0.1'});
				}
			});
			...
			setTimeout(function(){
				$(thisItem).closest('.slider-animation-wrap').find('.js-movement-slider-item').each(function(index){
					if (index%2 === 0) {
						TweenLite.to($(this), normalSpeed, {ease: Power4.easeOut, x: '0%'});
					} else {
						TweenLite.to($(this), delaySpeed, {ease: Power3.easeOut, x: '0%', delay: '0.1'});
					}
				});
			}, slideShow);
		}
	...
	$('.-slider-init').on('afterChange', function(){
		sliderInAction = false;
	});
